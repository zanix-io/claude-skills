...
| [`zanix-dependency-direction`](.claude/skills/ecosystem/zanix-dependency-direction/SKILL.md) | Shared | Own description explicitly includes `@zanix/app`/`@zanix/space` "as consumers of the same tiers"; `zanix-feature-builder` loads it for any new cross-package import in a consumer project. Its duplicate-resolved-version axis is loaded by `consumer-conventions-reviewer` too (conditionally, on a `@zanix/*` range bump), since that hazard applies to consumer apps directly, not just the 12 library repos. |
