...
description: Real, confirmed-via-repro footguns in TEMPORARILY linking an unpublished/pre-release sibling package via a raw relative-path `deno.jsonc` `imports` override (not the `links` array) — `scopes` alias-name collisions, the exact prefix depth a `scopes` key needs to actually take effect, per-subpath override coverage gaps, and a raw local-path override silently auto-linking unrelated sibling directories too. Applies to any Deno monorepo/workspace testing a fix across sibling repos before publish — not Zanix-architecture-specific, though every confirmed case here happened in the Zanix ecosystem. Also covers two general `@deno/loader` `Workspace`/`Loader` construction gotchas unrelated to the TEMP-linking technique itself — a `newestDependencyDate` type/runtime mismatch, and a `configPath: undefined` auto-discovery collision — worth checking whenever any Deno tool builds its own `Workspace`, not just when testing a TEMP link. Split out of `deno-lazy-dependency-pattern` (2026-08-25) once that skill's own TEMP-linking content grew into a second, genuinely separate responsibility from the lazy-dependency pattern itself. Use whenever adding, debugging, or reviewing a TEMP local-path override plus its paired `scopes` block, when a fix that should be live via such a link doesn't seem to be taking effect, or when building/debugging any tool's own `@deno/loader` `Workspace` construction.
---
...
## Two general `@deno/loader` `Workspace` gotchas — not about the TEMP-linking technique itself, but the same underlying library

Both confirmed live in `@zanix/cli`'s own resolver (`src/commands/space/shared/import-project-module.ts`), which builds a `Workspace` per discovered config path to resolve a served project's own bare specifiers correctly — not while testing a TEMP sibling link, but any Deno tool constructing its own `Workspace`/`Loader` hits the same two gotchas.

- **`WorkspaceOptions.newestDependencyDate` is typed `Date`, but the WASM binding rejects a real `Date` instance at runtime.** Confirmed live, `@deno/loader@0.5.0`: passing an actual `Date` throws `Failed deserializing workspace options.: Error: invalid type: JsValue(Date), expected an RFC 3339 formatted date and time string` — only the ISO string form is accepted, despite the type signature. Worked around in `import-project-module.ts:195-204` (`getLoaderFor`) by calling `.toISOString()` and casting the result back to `Date` to satisfy the (wrong) declared type — a genuine library type/runtime mismatch, not a mistake in the caller's own `Date`-returning helper (`readNewestDependencyDate`, `import-project-module.ts:165-180`, kept `Date`-typed since that's the semantically correct return for every OTHER caller). Also worth knowing: `@deno/loader`'s own config-file discovery reads a project's `imports`/`compilerOptions` automatically but never translates a `deno.json(c)`'s own `"minimumDependencyAge"` field into `newestDependencyDate` on its own — `readNewestDependencyDate()` does that translation by hand (a plain number as minutes, or an absolute RFC3339 cutoff string), and needs to be passed to every `Workspace` this kind of tool constructs. **The rule**: never pass a real `Date` to `newestDependencyDate` — always `.toISOString()` it first, regardless of what the type signature claims. Re-check against the installed `@deno/loader` version before assuming this is still true — a type/runtime mismatch like this is exactly the kind of thing a future release corrects.

- **`configPath: undefined` means "auto-discover from `Deno.cwd()`," not "no config" — and that auto-discovery can silently land on the CALLING project's own config instead of the tool's.** Per `@deno/loader`'s own `WorkspaceOptions.configPath` doc, omitting it triggers config-file discovery starting from `Deno.cwd()`. A tool building a SEPARATE `Workspace` to represent its OWN identity/config (distinct from the file it's actually resolving on behalf of) hits a real footgun whenever it runs with a DIFFERENT project as `Deno.cwd()` and has no local config of its own to point at (`configPath === undefined`, e.g. because the tool itself was loaded from a remote `jsr:`/`https:` specifier with no local checkout): the resulting `Workspace` doesn't mean "no config" — it silently auto-discovers the CALLING/SERVED project's own nearest `deno.json`, becoming structurally IDENTICAL to a loader built explicitly FOR that project. Confirmed live in `@zanix/cli`'s `getCliLoader()` (`import-project-module.ts:211-240`): under any genuine global install (`cliConfigPath` is `undefined`), the loader meant to answer "does `@zanix/cli` itself have an opinion on this specifier" ends up identical to the served project's own loader, since both discover the same nearest `deno.json` from the same `Deno.cwd()`. Trusting any successful resolution from that loader as "genuinely the tool's own identity" is wrong — it can just as easily be the project's own bare local alias resolving "successfully" through the wrong loader (`@zanix/cli`'s own fix for exactly this, `cliLoaderHasNoRealLocalAnswer`, `import-project-module.ts:288-293`, is specific to how that package tells the two cases apart — cite it as a worked example, not a reusable helper). **The rule**: before trusting a `Workspace`/`Loader` built with `configPath: undefined` as representing "this tool's own answer" rather than "no config at all," check what `Deno.cwd()` actually is when that `Workspace` gets constructed — if it's a directory the tool doesn't own, the loader is really speaking for whatever project lives there.
...
- A general audit of `@deno/loader` API usage beyond the two confirmed gotchas above — this skill
  tracks those two because they were hit for real, not because it's a comprehensive `@deno/loader`
  reference; check that package's own current docs for anything not covered here.
